...
#Definición de parametros para el problema
path = "Datos.xlsx"
# path = "muros GHU.xlsx"
# path = "muros GDHU.xlsx"

# path = "muros 0.01m aisl.xlsx"
# path = "muros 1m aisl.xlsx"
# path = "muros 0.01m aisl+Tierra.xlsx"
# path = "muro 1.00m aisl+Tierra.xlsx"
# path = "muros sin piso +60m en muro.xlsx"

T=pd.read_excel(path,sheet_name='temperatura')
muro=pd.read_excel(path,sheet_name='muros')
    ...
    if len(floor) == 1:
        ...
        if floor_ct == final_index:
            if floor_fl == final_index - 1:
                ...
                order_final[floor_fl], order_final[final_index-1] = order_final[final_index-1], order_final[floor_fl]
    ...
    if k==0:                                                 
        ...
        for j in range(0,lenpared):
            ...
            if j==lenpared-1 and ordenfinal[j]=="CT":
                ...
                e[(axu1),(axu1)]=kpl[len(kpl)-int(nodos[j]),j]/dt+hi[j]+hpl[len(hpl)-int(nodos[j])+2,j]
                ...
                if ordenfinal[j]=="IN" or ordenfinal[j]=="AD":
                    ...
                    e[(axu1),(axu1)]=kpl[len(kpl)-int(nodos[j]),j]/dt+hre[1]+he[j]+hpl[len(hpl)-int(nodos[j])+2,j]
                    e[(axu1),(axu1+1)]=-hpl[len(hpl)-int(nodos[j])+2,j]
                    axu1=axu1+int(nodos[j])           
...
#                                 #####Elementos de la diagonal
        ...
        for j in range(1,lenpared+1):
            ...
            e[axu2-2,axu2-2]=kpl[k,j-1]/dt+hi[j-1]+hpl[len(hpl)-1,j-1]+hri[1]-hri[1]*area[j-1]/Atot
...
#   #############################################################################
        ...
        for j in range(0,lenpared):
            for p in range(1,int(nodos[j])-2):
                ...
                e[axu8,axu8]=kpl[int(cuenta[j])+p,j]/dt+hpl[int(cuenta[j])+p-1,j]+hpl[int(cuenta[j])+p,j]
...
for i in x:  
    ...
    #Balance energetico    
    c2 = Hve[i]*theta_sup[i]+Hve_i[i]*t_e[i]+Htr*t_e[i]+f_int*flu_int[i]+f_sol*flu_sol[i]+f_HC*flu_HC[i]
    ...
    for j in range(0,lenpared):
        ...
        if ordenfinal[j]=="N" or ordenfinal[j]=="NE" or ordenfinal[j]=="E" or ordenfinal[j]=="SE" or ordenfinal[j]=="S" \
        or ordenfinal[j]=="SO" or ordenfinal[j]=="O" or ordenfinal[j]=="NO" or ordenfinal[j]=="N" or ordenfinal[j]=="HR" \
        or ordenfinal[j]=="FL":
            if ordenfinal[j]=="N":
...
###############################################################################            
    ...
    re[n]=kpl_v[0]*T_old[i,n]/dt+(hce_v+hre[1])*t_e[i]+PHISOL_VENTANA[i]
        #Superficie interna
    re[n+1]=kpl_v[1]*T_old[i,n+1]/dt+(1.0/Atot)*((1.0-f_int)*flu_int[i]+(1.0-f_sol)*flu_sol[i]+(1.0-f_HC)*flu_HC[i])
        #Balance interior
    re[n+2]=kpl_v[2]*T_old[i,n+2]/dt+Hve[i]*theta_sup[i]+Hve_i[i]*t_e[i]+Htr*t_e[i]+f_int*flu_int[i]+f_sol*flu_sol[i]+f_HC*flu_HC[i]
    ...
    if CLIMA[i]==1:
        ...
        # Step 2
            ...
            #Factores dependientes solo de la hora
            #Nodo superficie interior
            b2 = (1.0/Atot)*((1.0-f_int)*flu_int[i]+(1.0-f_sol)*flu_sol[i]+(1.0-f_HC)*flu_HC_upper)
            #Balance energetico    
            c2 = Hve[i]*theta_sup[i]+Hve_i[i]*t_e[i]+Htr*t_e[i]+f_int*flu_int[i]+f_sol*flu_sol[i]+f_HC*flu_HC_upper
        ...
        ###############################################################################            
            ...
            re[n]=kpl_v[0]*T_old[i,n]/dt+(hce_v+hre[1])*t_e[i]+PHISOL_VENTANA[i]
                #Superficie interna
            re[n+1]=kpl_v[1]*T_old[i,n+1]/dt+(1.0/Atot)*((1.0-f_int)*flu_int[i]+(1.0-f_sol)*flu_sol[i]+(1.0-f_HC)*flu_HC_upper)
                #Balance interior
            re[n+2]=kpl_v[2]*T_old[i,n+2]/dt+Hve[i]*theta_sup[i]+Hve_i[i]*t_e[i]+Htr*t_e[i]+f_int*flu_int[i]+f_sol*flu_sol[i]+f_HC*flu_HC_upper            
...
###### Piso ventilado
    ...
    re2[n]=kpl_v[0]*T_old4[i,n]/dt+(hce_v+hre[1])*t_e[i]+PHISOL_VENTANA[i]
        #Superficie interna
    re2[n+1]=kpl_v[1]*T_old4[i,n+1]/dt+(1.0/Atot)*((1.0-f_int)*flu_int[i]+(1.0-f_sol)*flu_sol[i])
        #Balance interior
    re2[n+2]=kpl_v[2]*T_old4[i,n+2]/dt+Hve[i]*theta_sup2[i]+Hve_i[i]*t_e[i]+Htr*t_e[i]+f_int*flu_int[i]+f_sol*flu_sol[i]
    # Despeje de temperatura y guardado de temperaturas
...
valor+=conductividad.shape[1]+cp.shape[1]+densidad.shape[1]+espesor.shape[1]+dx.shape[1]+kpl.shape[1]+hpl.shape[1]+Nd.shape[1]
...
def x_int_aire(densidad_aire, flujo_aire, x_a_e, GHU_DHU, G_int, G_abs, Volumen_int, dt, x_int_aire_t1):
    ...
    x_int_aire=(densidad_aire*flujo_aire*x_a_e+GHU_DHU+G_int-G_abs+densidad_aire*Volumen_int*x_int_aire_t1/dt)/(densidad_aire*flujo_aire+densidad_aire*Volumen_int/dt)
...
for i in range(0,L):
    ...
    G_HU[i]=G_HU_ld(densidad_a, flujo_aire[i], x_setmin[i], x_a_e[i], x_int_a[i], G_int[i], G_abs, Volumen_int, dt)
    G_DHU[i]=G_DHU_ld(densidad_a, flujo_aire[i], x_setmax[i], x_a_e[i], x_int_a[i], G_int[i], G_abs, Volumen_int, dt)
    if G_HU[i]!=0: 
        x_int_a[i+1]=x_int_aire(densidad_a, flujo_aire[i], x_a_e[i], G_HU[i], G_int[i], G_abs, Volumen_int, dt, x_int_a[i])
    elif G_DHU[i]!=0:
        x_int_a[i+1]=x_int_aire(densidad_a, flujo_aire[i], x_a_e[i], G_DHU[i], G_int[i], G_abs, Volumen_int, dt, x_int_a[i])
...
G_HU= CLIMA*G_HU
G_DHU= CLIMA*G_DHU
x_int_a[1:9505]= CLIMA*x_int_a[1:9505]




for i in range(0,len(e)):
    output[0:9504,i]=T_old[1:9505,i]
    
for i in range(0,len(e)):
    output[0:9504,i+len(e)]=T_old4[1:9505,i]
...
output2=np.zeros([L-744,11])
contador2=0
output2 , contador2 = f_list2(ID_Recinto, output2, contador2, 744)
output2 , contador2 = f_list2(hora2, output2, contador2, 744)
output2 , contador2 = f_list2(dia, output2, contador2, 744)
output2 , contador2 = f_list2(mes, output2, contador2, 744)
output2 , contador2 = f_list2(t_e, output2, contador2, 744)
output2 , contador2 = f_list2(theta_int_op2, output2, contador2, 744)
output2 , contador2 = f_list2(theta_int_op, output2, contador2, 744)
output2 , contador2 = f_list2(flu_HC, output2, contador2, 744)
output2 , contador2 = f_list2(flu_sol, output2, contador2, 744)
output2 , contador2 = f_list2(G_HU, output2, contador2, 744)
output2 , contador2 = f_list2(G_DHU, output2, contador2, 744)
...
muros3.append("Temperatura_operativa_con_clima")
muros3.append("Demanda")
muros3.append("G_HU")
muros3.append("G_DHU")


df2=T.iloc[744:9505,0:6].reset_index()
vectores=np.array([theta_int_op2[744:9505],theta_int_op[744:9505],flu_HC[744:9505],G_HU[744:9505],G_DHU[744:9505]]).T
df3=pd.DataFrame(vectores, columns = ['Temperatura op free float','Temperatura op','Demanda','G_HU','G_DHU'])

df4=pd.concat([df2,df3], axis=1)
promedios=df4.groupby(['month', 'hours/week']).mean()
promedios=promedios.reset_index()

output3=np.zeros([288,9])
contador3=0
output3 , contador3 = f_list2(ID_Recinto[0:288], output3, contador3, 0)
...
output4=np.zeros([L-744,10])
contador4=0
output4 , contador4 = f_list2(ID_Recinto, output4, contador4, 744)
output4 , contador4 = f_list2(hora2, output4, contador4, 744)
output4 , contador4 = f_list2(dia, output4, contador4, 744)
output4 , contador4 = f_list2(mes, output4, contador4, 744)
output4 , contador4 = f_list2(p_sat, output4, contador4, 744)
output4 , contador4 = f_list2(x_setmin, output4, contador4, 744)
output4 , contador4 = f_list2(x_setmax, output4, contador4, 744)
output4 , contador4 = f_list2(x_int_a, output4, contador4, 745)
output4 , contador4 = f_list2(G_HU, output4, contador4, 744)
output4 , contador4 = f_list2(G_DHU, output4, contador4, 744)
...
if Informes==0:
    ...
    b_file = open (ID1+" informe simple promedios.txt", "a")
    headers3=""
    ...
    b_file.close()   
    # c_file = open (ID1+" informe humedad.txt", "a")
    # headers4=""
    ...
    file = open (ID1+" informe simple.txt", "a")
    headers2=""
    ...
    a_file = open(ID1+" informe completo.txt", "a")
    headers=""
...
# fin=time.time()
# print("Tiempo de ejecución del cálculo")
# print(fin-inicio)
