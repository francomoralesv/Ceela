...
const RecintoCreateEdit: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Validar que no existan más de 3 recintos
      ...
      if (checkResponse.ok) {
        const existingRecintos = await checkResponse.json();
        if (Array.isArray(existingRecintos) && existingRecintos.length >= 3) {
          notify("No se puede crear más de 3 recintos por proyecto", "error");
          return;
        }
      }
  ...
  return (
    <>
      ...
      </Card>      {/* Nueva Card para "Características térmicas de la envolvente"
